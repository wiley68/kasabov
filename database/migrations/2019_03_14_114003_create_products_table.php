...
class CreateProductsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
            ...
            $table->integer('first_color')->default(0);
            $table->integer('second_color')->default(0);
            $table->enum('age', array('child', 'adult'));
            $table->enum('pol', array('man', 'woman'));
            $table->enum('condition', array('new', 'old'));
            $table->integer('send_id')->default(0);
            $table->integer('send_from_id')->default(0);
            $table->float('price_send');
