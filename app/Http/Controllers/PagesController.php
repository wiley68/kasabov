...
class PagesController extends Controller
{
    ...
    public function editPolitikaBiskvitki(Request $request){
        $page = Page::first();
        if ($request->isMethod('post')){
            $page->value_pb = $request->input('value_pb');
            $page->save();
        }
        return view('admin.pages.edit_politika_biskvitki')->with([
            'page'=>$page
        ]);
    }

    public function politkaBiskvitki(){
        $holidays_count = Holiday::where(['parent_id'=>0])->count();
        if ($holidays_count >= 5){
            $holidays_count = 5;
        }
        ...
        $categories_top = Category::where(['parent_id'=>0])->take($categories_top_count)->get();
        $page = Page::first();
        return view('politika_biskvitki')->with([
            'holidays'=>$holidays,
            'cities'=>$cities,
            'categories_top'=>$categories_top,
            'page'=>$page
